...
class categoryClass:
    def __init__(self,root):
     ...
     #===Category Details===
        ...
        self.lb1_im2=Label(self.root,image=self.im1,bd=2,relief=RAISED)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category is already present, try with different one.",parent=self.root)
                else:
    ...
    def show(self):
        ...
        try:
            ...
            for rows in rows:
                self.category_table.insert('',END,values=rows)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
